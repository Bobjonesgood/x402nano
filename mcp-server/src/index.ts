...
server.registerTool(
  ...
  async ({ limit }) => {
    try { return result(await client.listTrendingMarkets(limit)); } catch (error) { return failure(error); }
  }
);
...
server.registerTool(
  ...
  async () => {
    try { return result(await client.getPricing()); } catch (error) { return failure(error); }
  }
);
...
server.registerTool(
  ...
  async ({ slug }) => {
    try { return result(await client.getMarketBrief(slug)); } catch (error) { return failure(error); }
  }
);
...
server.registerTool(
  ...
  async ({ slug, since }) => {
    try { return result(await client.getMarketDelta(slug, since)); } catch (error) { return failure(error); }
  }
);
